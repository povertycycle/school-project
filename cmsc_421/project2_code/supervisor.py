...
import proj3					# File containing your programs for Project 2


def main(problem=rhook16b, time_limit=15, draw=1):
	"""
	...
	# If proj3 includes an initialization procedure, call it to cache some data
	if 'initialize' in dir(proj3):
		print('Calling proj3.initialize.')
		...
		p.join(150)
		if p.is_alive():
			print('\nWarning: terminating proj3.initialize at 15 seconds.')
			print('This means its output may be incomplete.')
		p.terminate()
	else:
		print("Note: proj3.py doesn't contain an initialize program.")
	...
	while True:
		...
		(u, v, ok) = get_proj3_choice((x,y), (u,v), f_line, walls, time_limit)
		if ok == False:
		...
		print('proj3 chose velocity {}, steering error is {}, result is {}'.format( \
			(u,v), error, (xnew,ynew)))
		edge = ((x,y), (xnew, ynew))
		if draw:
			draw_edge(edge, 'red')
