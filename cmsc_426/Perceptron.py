...
# test run  = python Perceptron.py *filename
# filename has to contain neg and pos
# this will save weight through pickle and print accuracy.
...
def compute_accuracy(weights, xs, labels):
    ...
    for (x, y) in zip(xs, labels):
        print(dot(weights, x))
        if dot(weights, x) < 1.1 and y == -1:
            n += 1
            correct = correct + 1
        elif dot(weights, x) >= 1.1 and y == 1:
            p += 1
            correct = correct + 1
        total = total + 1
    print(n, p)
...
w = [0.0] * len(hdatapoints[0])
w2 = run_epoch(w, hdatapoints, labels)
percentage, all = compute_accuracy(w2, hdatapoints, labels)
print(percentage/all*100)

output = open("perception.pkl", "wb")
pickle.dump({'weights': w2, 'name':'wsentosa', 'class': 426}, output)
output.close()
