...
import cmsc420.sortedmap.PMQuadtree1.White;

public class PMQuadtree3 implements PMQuadtree {
	...
	public PMQuadtree3(int width, int height) {
		this.localSpatialWidth = width;
		this.localSpatialHeight = height;
		this.root = new PMQuadtree3.White(new Point2D.Float(0, 0), localSpatialWidth, localSpatialHeight);
	...
	class Black implements PMNode {
		...
		public Black(PMNode black, Point2D.Float origin, int width, int height) {
			...
			TreeSet<Road> roadList = black.getRoads();
	...
	class Gray implements PMNode {
		...
		/* Constructor
		/* ========================================================================================================= */	
		public Gray(ArrayList<City> cityList, TreeSet<Road> roadList, ArrayList<Airport> airportList, 
				ArrayList<Terminal> terminalList, int width, int height, Point2D.Float origin) { 
			this.origin = origin;
			this.spatialHeight = height;
			this.spatialWidth = width;
			createOrigins();
		...
		private PMNode getTheBlackNode() {
			if (isOnlyBlack()) {
				for (int i = 0; i < 4; i++) {
					if (children[i].getNodeType() == PMType.BLACK) {
						return children[i];
					}
				}
			}
		...
		public PMNode removeRoad(Road road) {
			...
			while (iter.hasNext()) {
				City c = iter.next();
				if (road.IsEither(c)) c.getRoadList().remove(road);
